...
import {
  ArrowLeft, GripVertical, Plus, X, FileDown,
  Search, Edit3, Check, ChevronUp, ChevronDown, Music2
} from 'lucide-react'
import {
  DndContext,
  closestCenter,
...
import { Button, Input, Badge, EmptyState, ErrorState, Skeleton, Select, Tooltip, Modal } from '../components/ui'

export default function SetlistView() {
  const { id } = useParams()
  const { setlist, loading, error, reload, addSong, removeSong, updateSlot, reorder, rename } = useSetlist(id)
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      containers.forEach(c => { try { document.body.removeChild(c) } catch {} })
    } catch (e) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Link to="/setlists" className="flex items-center gap-1 text-xs text-[var(--color-ink-muted)] hover:text-[var(--color-ink)] transition-colors">
          ...
          {renaming ? (
            <div className="flex items-center gap-2">
              <Input
                ...
                autoFocus
              />
              <Button variant="primary" size="icon-sm" onClick={handleRename}><Check size={13} /></Button>
              <Button variant="ghost" size="icon-sm" onClick={() => setRenaming(false)}><X size={13} /></Button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <h1 className="font-display text-2xl text-[var(--color-ink)] truncate">{setlist.name}</h1>
              <Button variant="ghost" size="icon-sm" onClick={handleStartRename} title="Rename setlist">
                <Edit3 size={13} />
              </Button>
            </div>
          )}
        ...
        <div className="lg:col-span-2 space-y-2">
          ...
          {slots.length === 0 ? (
            <EmptyState
              ...
              action={
                <Button variant="secondary" size="sm" onClick={() => setAddPanelOpen(true)}>
                  <Plus size={13} /> Add song
                </Button>
              }
            />
        ...
        {addPanelOpen && (
          <div className="border border-[var(--color-border)] rounded-lg bg-[var(--color-bg-warm)] p-3 space-y-3 h-fit sticky top-20">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="icon-sm" onClick={() => setAddPanelOpen(false)}>
                <X size={13} />
              </Button>
            </div>
            <div className="relative">
              <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[var(--color-ink-muted)] pointer-events-none" />
              <input
                type="text"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 ml-2">
                        ...
                        <Plus size={11} className="text-[var(--color-ink-muted)] group-hover:text-[var(--color-accent)] transition-colors" />
                      </div>
                    </div>
                    {song.artist && (
                      <span className="text-[10px] text-[var(--color-ink-muted)]">{song.artist}</span>
                    )}
                  </button>
                ))
              )}
...
function SortableSlot({ slot, index, onRemove, onUpdateSlot }) {
  ...
  const [localKey, setLocalKey] = useState(slot.chosen_key || slot.song?.original_key || '')
  const [localCapo, setLocalCapo] = useState(slot.capo || 0)

  const displayKey = localKey || slot.song?.original_key
  const capoHint = displayKey && localCapo > 0
    ? getCapoDisplay(displayKey, localCapo)
    : null
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-2 flex-wrap">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-[var(--color-ink-muted)]">Key</span>
            <Select
              value={displayKey || ''}
            ...
            {slot.song?.original_key && displayKey !== slot.song.original_key && (
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-[var(--color-ink-muted)]">Capo</span>
