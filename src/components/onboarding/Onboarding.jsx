...
import {
  LayoutGrid, Plus, FileUp, ListMusic, Guitar, BarChart3,
  Music2, Zap, Moon, Type, CloudOff, ArrowRightLeft, Hash,
  Sparkles, ChevronDown, BookOpen, Eye, Shield, ShieldCheck, Play,
} from 'lucide-react'
    ...
    <div className="flex gap-3">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {step.to && <span className="text-[10px] text-[var(--color-ink-muted)]">↗</span>}
        </div>
        <p className="text-xs text-[var(--color-ink-soft)] mt-0.5 leading-relaxed">{step.body}</p>
      </div>
    </div>
  )
...
function RoleNote({ role }) {
  ...
  return (
    <div className="flex items-start gap-2 p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-warm)]">
      ...
      <p className="text-xs text-[var(--color-ink-soft)] leading-relaxed">{note.text}</p>
    </div>
  )
}
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={null} className="max-w-lg">
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-display text-lg text-[var(--color-ink)] flex items-center gap-1.5">
              Welcome to ChordVault <Sparkles size={15} className="text-[var(--color-accent)]" />
            </h2>
            <p className="text-xs text-[var(--color-ink-muted)]">Your team's chord charts, setlists and stage view — here's the quick tour.</p>
          </div>
        </div>
        ...
        {auth.isLoggedIn ? (
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-warm)]">
            ...
            <p className="text-xs text-[var(--color-ink-soft)] leading-relaxed">
              You're not signed in — songs stay only in this browser. Sign in from the top bar to sync your library across devices.
            </p>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-1 border-t border-[var(--color-border)]">
          <span className="text-[10px] text-[var(--color-ink-muted)]">Find this again any time in Profile.</span>
    ...
    <div className="border border-[var(--color-border)] rounded-lg bg-[var(--color-bg-warm)] overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-sm font-semibold text-[var(--color-ink)]">How ChordVault works</h2>
          <p className="text-[11px] text-[var(--color-ink-muted)]">A quick guide to every section and what your role can do</p>
        </div>
        <ChevronDown
          size={16}
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-[var(--color-border)] pt-4">
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button variant="secondary" size="sm" onClick={() => setReplayOpen(true)}>
