...
import {
  X, ChevronLeft, ChevronRight, Play, Pause, Gauge, Music2, Hash, Type,
} from 'lucide-react'
import { createPortal } from 'react-dom'
import { semitonesFromKeyToKey, transposeKey, getCapoDisplay } from '../../lib/transposition'
...
function slotDisplay(slot) {
  ...
  const semitones = slot.chosen_key && song?.original_key
...
export default function SetlistPerformer({ slots, onClose }) {
  ...
  useKeyboardControls({ onNext, onPrev, onToggle: toggleScroll, enabled: true })
      ...
      <div className="flex items-center gap-3 px-4 h-12 border-b border-[var(--color-border)] shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-2">
            <h2 className="font-mono text-sm font-bold text-[var(--color-ink)] truncate">{song.title}</h2>
            {displayKey && <Badge variant="key">{displayKey}{capo > 0 ? ` · capo ${capo}` : ''}</Badge>}
          </div>
          {song.artist && <p className="text-[10px] text-[var(--color-ink-muted)] truncate">{song.artist}</p>}
        </div>

        {/* Font size */}
        <div className="hidden sm:flex items-center gap-1">
          <Type size={12} className="text-[var(--color-ink-muted)]" />
          <Button variant="ghost" size="icon-sm" onClick={() => setFontSize((s) => Math.max(10, s - 1))} disabled={fontSize <= 10}>−</Button>
          <span className="w-8 text-center font-mono text-[11px]">{fontSize}px</span>
        ...
        <Button variant="ghost" size="icon-sm" onClick={onClose} title="Exit (Esc)">
          <X size={16} />
        </Button>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 px-3 sm:px-4 h-16 border-t border-[var(--color-border)] bg-[var(--color-bg-warm)] shrink-0">
        ...
        <div className="flex items-center gap-3 sm:gap-4 overflow-x-auto">
          <div className="flex items-center gap-1.5 shrink-0">
            <Tooltip content={scrolling ? 'Pause scroll (space)' : 'Auto-scroll (space)'}>
              <Button variant={scrolling ? 'primary' : 'secondary'} size="icon-sm" onClick={toggleScroll}>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <Button variant="ghost" size="icon-sm" onClick={() => nudgeBpm(-1)} disabled={bpm <= 30}>−</Button>
            ...
            <Button variant="ghost" size="icon-sm" onClick={() => nudgeBpm(1)} disabled={bpm >= 300}>+</Button>
        ...
        {/* Next */}
        <Button variant="secondary" size="md" onClick={onNext} disabled={index >= playable.length - 1}>
