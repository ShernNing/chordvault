...
async function computePageLayout(editedSlots) {
  ...
  const pages = []
  let leftCol = [], rightCol = [], leftH = 0, rightH = 0, useRight = false

  const flushPage = () => {
    if (!leftCol.length) return
    pages.push({ type: 'multi', left: [...leftCol], right: [...rightCol] })
    leftCol = []; rightCol = []; leftH = 0; rightH = 0; useRight = false
  }
    ...
    if (!d.fitsHalfPage || d.estimatedH > PAGE_COL_HEIGHT) {
      ...
      if (!useRight) {
        ...
        if (leftH === 0 || newH <= PAGE_COL_HEIGHT) {
          leftCol.push(d); leftH = leftH === 0 ? h : newH
        } else {
          useRight = true; rightCol.push(d); rightH = h
        }
      } else {
        const newH = rightH + (rightH > 0 ? SONG_GAP : 0) + h
        if (rightH === 0 || newH <= PAGE_COL_HEIGHT) {
          rightCol.push(d); rightH = rightH === 0 ? h : newH
        } else {
          flushPage()
          leftCol.push(d); leftH = h
        }
      }
    }
  }
...
export default function SetlistFullEditor({
  ...
  const handleEditLine = useCallback((slotId, sectionId, lineIdx, text) => {
    mutateSections(slotId, secs =>
      secs.map(sec => {
        ...
        const nl = [...sec.lines]; nl[lineIdx] = lineWithNewText(nl[lineIdx], text)
        return { ...sec, lines: nl }
      })
    )
  }, [mutateSections])
  ...
  const handlePasteAfter = useCallback((slotId, afterSectionId) => {
    ...
    mutateSections(slotId, secs => {
      ...
      const out = [...secs]; out.splice(idx + 1, 0, pasted)
      return out
    })
    setClipboard(null)
  ...
  return (
    <div className='fixed inset-0 z-50 flex flex-col' style={{ backgroundColor: '#c8c8c8' }}>
      {/* Top bar */}
      ...
      <div className='flex-1 overflow-y-auto py-8'>
        {layoutBusy ? (
          <div className='flex flex-col items-center justify-center h-full gap-3'>
            <RefreshCw size={22} className='animate-spin text-[var(--color-ink-muted)]' />
            <span className='text-sm text-[var(--color-ink-muted)]'>Computing page layout…</span>
          </div>
        ) : (
...
function EditorPage({ pageNum, page, slotById, clipboard, onReorderSections, onEditLine, onCutSection, onPasteAfter }) {
  const pageStyle = {
    width: '794px',
    minHeight: '1123px',
    ...
    <div style={pageStyle}>
      ...
      <div style={hasTwoColumns ? { display: 'flex', gap: '32px' } : {}}>
        <div style={{
          ...
          display: 'flex', flexDirection: 'column', gap: `${SONG_GAP}px`,
        }}>
          {makeCol(leftData)}
        </div>
        {hasTwoColumns && (
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: `${SONG_GAP}px` }}>
            {makeCol(rightData)}
          </div>
        )}
      </div>
...
function PageLabel({ num }) {
  return (
    <div style={{
      ...
      fontSize: '10px', color: '#ccc', userSelect: 'none', pointerEvents: 'none',
    }}>
      p.{num}
    </div>
  )
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '8px' }}>
        <span style={{ fontSize: '19px', fontWeight: '700', display: 'block', wordBreak: 'break-word' }}>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={sectionIds} strategy={verticalListSortingStrategy}>
          {clipboard && <PasteZone label='Paste at top' onPaste={() => onPasteAfter(null)} />}
          {slot.sections.map(section => (
            <React.Fragment key={section.id}>
              ...
              />
              {clipboard && <PasteZone label='Paste here' onPaste={() => onPasteAfter(section.id)} />}
            </React.Fragment>
          ))}
        </SortableContext>
      </DndContext>
...
function EditableSection({ section, onEditLine, onCut }) {
  ...
  return (
    ...
    >
      {(hovered || isDragging) && (
        <div style={{
          ...
          <button
            ...
            style={{ color: '#aaa', cursor: 'grab', lineHeight: 0, padding: '2px' }}
            {...attributes}
            {...listeners}
          >
          ...
          <button
            title='Cut section'
            style={{ color: '#aaa', cursor: 'pointer', lineHeight: 0, padding: '2px' }}
            onClick={onCut}
      ...
      {section.header && (
        <EditableLineItem line={section.header} lineIdx={-1} onCommit={t => onEditLine(-1, t)} />
      )}
      {section.lines.map((line, li) =>
        line.type === 'blank'
          ? <div key={li} style={{ height: '8px' }} />
          : <EditableLineItem key={li} line={line} lineIdx={li} onCommit={t => onEditLine(li, t)} />
      )}
    </div>
  )
}
...
function EditableLineItem({ line, onCommit }) {
  ...
  const baseStyle = line.type === 'section_header' ? SECTION_HEADER_STYLE
    : line.type === 'chord_line' ? CHORD_STYLE
    : LYRIC_STYLE

  const hoverBg = line.type === 'chord_line' ? '#eff6ff'
    : line.type === 'section_header' ? '#f3f4f6'
    : '#f9fafb'
  ...
  return (
    <span
      ...
      onMouseOut={e => e.currentTarget.style.background = 'transparent'}
    >
      {displayText || (line.type === 'section_header'
        ? <span style={{ color: '#bbb', fontStyle: 'italic' }}>Section header</span>
        : ' '
      )}
    </span>
  )
