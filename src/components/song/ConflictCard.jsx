...
export default function ConflictCard({ conflict, onChange, incomingLabel = 'Importing' }) {
  ...
  const [incContent, setIncContent] = useState(importedSong.rawContent || importedSong.raw_content || '')
    ...
    <div className="border border-amber-300 dark:border-amber-700 rounded-lg overflow-hidden">
      ...
      {!showEditor && (
        <div className="grid grid-cols-2 divide-x divide-[var(--color-border)] text-xs">
          ...
          <div className="p-3 bg-[var(--color-bg)]">
            <p className="font-medium text-[var(--color-ink-muted)] uppercase tracking-wide text-[10px] mb-1">{incomingLabel}</p>
      ...
      {showEditor && (
        <div className="p-4 border-b border-[var(--color-border)] bg-[var(--color-bg)]">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-semibold text-[var(--color-ink-muted)] uppercase tracking-wide border-b border-[var(--color-border)] pb-1">{incomingLabel}</p>
      ...
      <div className="px-4 py-3 bg-[var(--color-bg)] border-t border-[var(--color-border)] space-y-2.5">
        ...
        {resolution === 'keep-both' && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-[var(--color-ink-muted)] shrink-0">New title for {incomingLabel.toLowerCase()}:</span>
            <input
              type="text"
              value={newTitle}
