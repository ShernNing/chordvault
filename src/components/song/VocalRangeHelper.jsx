...
function NoteSelect({ value, onChange, className = "" }) {
  return (
    <Select
      ...
      className={`h-7 w-16 text-xs py-0 ${className}`}
    >
      {NOTE_OPTIONS.map((n) => (
 ...
 */
export default function VocalRangeHelper({
  songId,
  originalKey,
  currentCapo = 0,
  onApply,
}) {
  const [voice, setVoice] = useLocalStorage("cv-vocal-range", {
    low: "A2",
    high: "C4",
  });
  const [songRange, setSongRange] = useLocalStorage(
    `cv-song-range-${songId}`,
    null,
  );
  ...
  const ready =
    songRange &&
    Note.midi(songRange.low) != null &&
    Note.midi(songRange.high) != null;
  ...
  return (
    <div className='no-print flex flex-col gap-3 px-3 py-3 bg-[var(--color-bg-warm)] border border-[var(--color-border)] rounded-lg'>
      ...
      <div className='flex items-center gap-2 flex-wrap'>
        <span className='text-xs text-[var(--color-ink-muted)] w-24'>
          My comfy range
        </span>
        <NoteSelect
          value={voice.low}
          onChange={(low) => setVoice((v) => ({ ...v, low }))}
        />
        <span className='text-xs text-[var(--color-ink-muted)]'>to</span>
        <NoteSelect
          value={voice.high}
          onChange={(high) => setVoice((v) => ({ ...v, high }))}
        />
      </div>
      ...
      <div className='flex items-center gap-2 flex-wrap'>
        <span className='text-xs text-[var(--color-ink-muted)] w-24'>
          Song's lowest / highest melody note
        </span>
        <NoteSelect
          value={songRange?.low || "C3"}
          onChange={(low) =>
            setSongRange((r) => ({ low, high: r?.high || "C4" }))
          }
        />
        <span className='text-xs text-[var(--color-ink-muted)]'>to</span>
        <NoteSelect
          ...
          onChange={(high) =>
            setSongRange((r) => ({ low: r?.low || "C3", high }))
          }
        />
      </div>
      ...
      {!ready ? (
        <p className='text-[11px] text-[var(--color-ink-muted)] italic'>
          Set the song's lowest and highest melody notes (in the original key),
          then fit.
        </p>
      ) : (
            ...
            {shift === 0 ? (
              ...
              <>
                ...
                {resultKey && (
                  <>
                    {" "}
                    → key of <strong className='font-mono'>{resultKey}</strong>
                  </>
                )}
                {!fullyFits && (
                  <span className='text-amber-600 dark:text-amber-400'>
                    {" "}
                    (best fit, song is wide)
                  </span>
                )}
              </>
            )}
