 ...
 *   onClose  () => void
 */
export default function ChordPlayer({ chords = [], bpm = 100, raised = false, onClose }) {
  ...
  const start = () => {
    ...
    const player = createProgressionPlayer({
      ...
      onEnd: () => {
        if (loopRef.current) { start() }
        else { setPlaying(false); setCurrent(-1) }
      },
    })
  ...
  const nextChord = current >= 0 && current + 1 < chords.length ? chords[current + 1] : null
    ...
    >
      <div className="pointer-events-auto flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[var(--color-bg-warm)] border border-[var(--color-border)] shadow-lg max-w-full">
        <Tooltip content={playing ? 'Stop' : 'Play chords'}>
          <Button variant={playing ? 'primary' : 'secondary'} size="icon-sm" onClick={toggle}>
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-[var(--color-ink-muted)] uppercase tracking-wide hidden sm:inline">Beats</span>
          <Select
            value={beatsPerChord}
        ...
        {onClose && (
          <>
            <div className="w-px h-6 bg-[var(--color-border)]" />
            <Button variant="ghost" size="icon-sm" onClick={onClose} title="Close chord player">
              <X size={14} />
            </Button>
          </>
        )}
