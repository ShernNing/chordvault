import { TSONRef, collectTSONRefs, replaceTSONRefs } from '../TSONRef';
describe('TSONRef', () => {
    ...
    describe('reconstruct', () => {
        ...
        test('handles unknown type gracefully', () => {
            const ref = new TSONRef('UnknownType', 'ref', 4);
            const data = new ArrayBuffer(4);
...
describe('collectTSONRefs', () => {
    ...
    test('collects TSONRefData from object', () => {
        const data = { $t: 'Uint8Array', $ref: 'ref-2', $size: 200 };
        const obj = { data };
        const refs = collectTSONRefs(obj);
        expect(refs).toHaveLength(1);
...
describe('replaceTSONRefs', () => {
    const mockResolver = jest.fn(async (ref) => {
    ...
    test('respects concurrency limit', async () => {
        let concurrent = 0;
        let maxConcurrent = 0;
        const slowResolver = jest.fn(async (ref) => {
            concurrent++;
        ...
        const refs = Array.from({ length: 10 }, (_, i) => new TSONRef('Uint8Array', `ref-${i}`, 4));
        const obj = Object.fromEntries(refs.map((r, i) => [`data${i}`, r]));
